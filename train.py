...
from trajectronpp import evaluation
from trajectronpp.argument_parser import args
from trajectronpp.model.trajectron import Trajectron
from trajectronpp.model.model_registrar import ModelRegistrar
from trajectronpp.model.model_utils import cyclical_lr
from trajectronpp.model.dataset import EnvironmentDataset, collate
...
def main():
    ...
    train_scenes_sample_probs = (
        train_env.scenes_freq_mult_prop if args.scene_freq_mult_train else None
    )
    ...
    for node_type_data_set in train_dataset:
        ...
        node_type_dataloader = utils.data.DataLoader(
            node_type_data_set,
            collate_fn=collate,
            pin_memory=False if torch.device(args.device) == torch.device("cpu") else True,
    ...
    if args.eval_every is not None:
        ...
        eval_scenes_sample_probs = (
            eval_env.scenes_freq_mult_prop if args.scene_freq_mult_eval else None
        )
        ...
        for node_type_data_set in eval_dataset:
            ...
            node_type_dataloader = utils.data.DataLoader(
                node_type_data_set,
                collate_fn=collate,
                pin_memory=False if torch.device(args.eval_device) == torch.device("cpu") else True,
