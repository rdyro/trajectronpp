...
from ..model import dynamics as dynamic_module
...
class MultimodalGenerativeCVAE(object):
    ...
    def create_node_models(self):
        ...
        self.add_submodule(
            self.node_type + "/hx_to_z", model_if_absent=nn.Linear(hx_size, self.latent.z_dim)
        )
        ...
        self.add_submodule(
            self.node_type + "/hxy_to_z", model_if_absent=nn.Linear(hxy_size, self.latent.z_dim)
        )
    ...
    def obtain_encoded_tensors(
        ...
        if (
            ...
            if self.log_writer and (self.curr_iter + 1) % 500 == 0:
                ...
                map_clone[
                    :, :, map_patch[1] - 5 : map_patch[1] + 5, map_patch[0] - 5 : map_patch[0] + 5
                ] = 1.0
        ...
        if self.hyperparams["edge_state_combine_method"] == "sum":
            ...
            if self.hyperparams["dynamic_edges"] == "yes":
                ...
                for edge_value in neighbors_edge_value:
                    op_applied_edge_mask_list.append(
                        torch.clamp(
                            torch.sum(edge_value.to(self.device), dim=0, keepdim=True), max=1.0
                        )
                    )
            ...
            if self.hyperparams["dynamic_edges"] == "yes":
                ...
                for edge_value in neighbors_edge_value:
                    op_applied_edge_mask_list.append(
                        torch.clamp(
                            torch.max(edge_value.to(self.device), dim=0, keepdim=True), max=1.0
                        )
                    )
                combined_edge_masks = torch.stack(op_applied_edge_mask_list, dim=0)
            ...
            if self.hyperparams["dynamic_edges"] == "yes":
                ...
                for edge_value in neighbors_edge_value:
                    op_applied_edge_mask_list.append(
                        torch.clamp(
                            torch.mean(edge_value.to(self.device), dim=0, keepdim=True), max=1.0
                        )
                    )
                combined_edge_masks = torch.stack(op_applied_edge_mask_list, dim=0)
    ...
    def encode_total_edge_influence(self, mode, encoded_edges, node_history_encoder, batch_size):
        if self.hyperparams["edge_influence_combine_method"] == "sum":
            ...
            if len(encoded_edges) == 0:
                ...
                _, state = self.node_modules[self.node_type + "/edge_influence_encoder"](
                    encoded_edges
                )
    ...
    def encode_node_future(self, mode, node_present, node_future) -> Tensor:
        """
    ...
    def encode_robot_future(self, mode, robot_present, robot_future) -> Tensor:
        """
    ...
    def q_z_xy(self, mode, x, y_e) -> Tensor:
        r"""
    ...
    def project_to_GMM_params(self, tensor) -> tuple(Tensor, Tensor, Tensor, Tensor):
        """
...
####################################################################################################
    ...
    def custom_predict(
        ...
        assert self.latent.p_dist.probs.shape == x.shape[:-1] + (1,) + self.latent.p_dist.event_shape
        ...
        probs = torch.sum(self.latent.p_dist.probs[:, 0, :] * z, -1)

        return our_sampled_future, z, probs
