...
def plot_vehicle_nice_mv(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0):
    ...
    for node in node_list:
        ...
        history = (
            histories_dict[node] + np.array([x_min, y_min]) + 5 * np.array([np.cos(h), np.sin(h)])
        )
        future = (
            futures_dict[node] + np.array([x_min, y_min]) + 5 * np.array([np.cos(h), np.sin(h)])
        )
        predictions = (
            prediction_dict[node] + np.array([x_min, y_min]) + 5 * np.array([np.cos(h), np.sin(h)])
        )
        if node.type.name == "VEHICLE":
            for t in range(predictions.shape[2]):
                sns.kdeplot(
